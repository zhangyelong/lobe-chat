...
本节将会介绍如何在 BianjieAI 中添加和使用一个自定义插件。
...
<Steps>
  ...
  ### 在 BianjieAI 角色设置中添加本地插件

  接下来进入到 BianjieAI 中，创建一个新的助手，并进入它的会话设置页：

  <Image alt="在 BianjieAI 角色设置中添加本地插件 S1" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265259643-1a9cc34a-76f3-4ccf-928b-129654670efd.png" />

  点击插件列表右侧的 <kbd>添加</kbd> 按钮，打开自定义插件添加弹窗：

  <Image alt="在 BianjieAI 角色设置中添加本地插件 S2" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265259748-2ef6a244-39bb-483c-b359-f156ffcbe1a4.png" />
  ...
  <Image alt="在 BianjieAI 角色设置中添加本地插件 S3" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265259964-59f4906d-ae2e-4ec0-8b43-db36871d0869.png" />

  完成添加后，在插件列表中就能看到刚刚添加的插件，如果需要修改插件的配置，可以点击最右侧的 <kbd>设置</kbd> 按钮进行修改。

  <Image alt="在 BianjieAI 角色设置中添加本地插件 S4" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265260093-a0363c74-0b5b-48dd-b103-2db6b4a8262e.png" />
  ...
  当完成这些操作后，你已经了解了添加自定义插件，并在 BianjieAI 中使用的基础流程。
</Steps>
...
3. `ui`：这个字段包含了插件的用户界面信息，指明了 BianjieAI 从哪个地址加载插件的前端界面。由于 BianjieAI 插件界面加载是基于 iframe 实现的，因此可以按需指定插件界面的高度、宽度。
4. `gateway`：这个字段指定了 BianjieAI 查询 api 接口的网关。BianjieAI 默认的插件网关是云端服务，而自定义插件的请求需要发给本地启动的服务，远端调用本地地址，一般调用不通。gateway 字段解决了该问题。通过在 manifest 中指定 gateway，BianjieAI 将会向该地址发送插件请求，本地的网关地址将会调度请求到本地的插件服务。发布到线上的插件可以不用指定该字段。
...
由于 BianjieAI 默认的插件网关是云端服务 `/api/plugins`，云端服务通过 manifest 上的 `api.url` 地址发送请求，以解决跨域问题。

针对自定义插件，插件请求需要发送给本地服务， 因此通过在 manifest 中指定网关 ([http://localhost:3400/api/gateway)，BianjieAI](http://localhost:3400/api/gateway\)，BianjieAI) 将会直接请求该地址，然后只需要在该地址下创建对应的网关即可。
...
[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) 包含了 BianjieAI 中插件网关的[实现](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts)，你可以直接使用该包创建网关，进而让 BianjieAI 访问到本地的插件服务。
...
BianjieAI 通过 `iframe` 实现插件 ui 的加载，使用 `postMessage` 实现主体与插件的通信。因此， 插件 UI 的实现方式与普通的网页开发一致，你可以使用任何你熟悉的前端框架与开发语言。
...
其中关于插件通信，我们在 [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) 提供了相关方法，用于简化插件与 BianjieAI 的通信。你可以通过 `fetchPluginMessage` 方法主动向 BianjieAI 获取当前消息的数据。关于该方法的详细介绍，参见：[fetchPluginMessage][fetch-plugin-message-url]。
