...
A BianjieAI plugin consists of the following components:
...
This section will introduce how to add and use a custom plugin in BianjieAI.
...
<Steps>
  ...
  ### Add Local Plugin in BianjieAI Role Settings

  Next, go to BianjieAI, create a new assistant, and go to its session settings page:

  <Image alt="Add Local Plugin in BianjieAI Role Settings S1" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265259643-1a9cc34a-76f3-4ccf-928b-129654670efd.png" />

  Click the <kbd>Add</kbd> button on the right of the plugin list to open the custom plugin adding popup:

  <Image alt="Add Local Plugin in BianjieAI Role Settings S2" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265259748-2ef6a244-39bb-483c-b359-f156ffcbe1a4.png" />
  ...
  <Image alt="Add Local Plugin in BianjieAI Role Settings S3" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265259964-59f4906d-ae2e-4ec0-8b43-db36871d0869.png" />
  ...
  <Image alt="Add Local Plugin in BianjieAI Role Settings S4" height={483} inStep src="https://github-production-user-asset-6210df.s3.amazonaws.com/28616219/265260093-a0363c74-0b5b-48dd-b103-2db6b4a8262e.png" />
  ...
  After completing these operations, you have understood the basic process of adding custom plugins and using them in BianjieAI.
</Steps>
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address BianjieAI loads the plugin's front-end interface. Since BianjieAI plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for BianjieAI to query the plugin's API interface. BianjieAI's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, BianjieAI will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for BianjieAI is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), BianjieAI> will directly request this address, and then only the corresponding gateway needs to be created at that address.

```ts
import { createBianjieAIPluginGateway } from '@lobehub/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createBianjieAIPluginGateway();
```

[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in BianjieAI [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing BianjieAI to access the local plugin service.
...
BianjieAI implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and BianjieAI. You can actively retrieve the current message data from BianjieAI using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from BianjieAI
